...
#define PART_CRCCHECK_EN      1             /* part 支持CRC 校验 */ 
...
do {                                             \
    ...
    self->stPartOps.hw_ops->hw_read(a, b, c);    \
    os_mutex_unlock(self->storage_mutex);        \
}while(0);


#define HW_WRITE(a, b, c)                        \
do {                                             \
    os_mutex_lock(self->storage_mutex);          \
    self->stPartOps.hw_ops->hw_write(a, b, c);   \
    os_mutex_unlock(self->storage_mutex);        \
}while(0);
...
do {                                             \
    os_mutex_lock(self->storage_mutex);          \
    self->stPartOps.hw_ops->hw_erase(a);         \
    os_mutex_unlock(self->storage_mutex);        \
}while(0);
...
#define STOR_PREFIX  "[STORAGE] [%s:%d] "
#define STOR_ALERT   CONNECT(KERN_ALERT,  STOR_PREFIX)
#define STOR_ERROR   CONNECT(KERN_ERROR,  STOR_PREFIX)
#define STOR_WARN    CONNECT(KERN_WARN,   STOR_PREFIX)
#define STOR_REPORT  CONNECT(KERN_REPORT, STOR_PREFIX)
#define STOR_INFO    CONNECT(KERN_INFO,   STOR_PREFIX)
#define STOR_TRACE   CONNECT(KERN_TRACE,  STOR_PREFIX)
...
#define STOR_PRINT(format, ...) os_printf_api(STOR_PREFIX format, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define STOR_DEBUG(format, ...)
#define STOR_PRINT(format, ...)
#endif

/* FLASH 数据打印---小端 */
#define FLASH_DATA_LITTLE_END 1
/* 是否展示升级进度条 */
#define USE_PROCESS_BAR       1

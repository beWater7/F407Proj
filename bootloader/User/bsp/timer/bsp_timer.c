...
/**
  ...
  * @param  无
  * @retval 无
  * @note   这里必须是全局符号（不能加 static），因为启动文件里中断向量表
  *         引用的是弱符号 TIM3_IRQHandler，如果这里是 static，链接器就无法
  *         用它覆盖弱符号，实际生效的还是 startup 文件里的 Default_Handler
  *         （死循环），导致 TIM3 中断一触发，程序就卡死在 Default_Handler
  *         里，主程序（含串口打印）根本没机会执行。
  */

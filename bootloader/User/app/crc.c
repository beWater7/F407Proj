...
/**
 * func: crc32_checksum()
 * descript: 计算标准 CRC32(poly=0x04C11DB7, init=0xFFFFFFFF, refin/refout=true, xorout=0xFFFFFFFF)
 * note: 直接使用文件内已经写死的静态表 CRC32_TABLE，不需要像通用 crc()/crc_table_init()
 *       那样在运行时重新生成一遍256项的表——那张表是常量，每次调用都重建纯属浪费CPU时间
 *       （分区头校验、OTA固件校验都会频繁调用这个函数，累积开销不可忽略）。
 * input/output: N/A
 * return value: void
 */

...
volatile uint16_t ymodem_rx_head = 0;
volatile uint16_t ymodem_rx_tail = 0;
...
/* */
void Ymodem_ProcessData(uint8_t *data, uint16_t len)
{
    int i = 0;
    ...
    printf("\r\n");
    // if(ymodem_rx_tail/1024)
    // {
    //     printf("recv\r\n");
    //     printf("%s\r\n", ymodem_ymodem_rx_buffer);
    //     printf("\r\n");
    // }
}


// void Ymodem_ProcessData(void)
// {
//     uint8_t c;
//     while (SerialKeyPressed(&c))
//     {
//         // 这里处理每个接收到的字节
//         printf("%c", c);
//         // 可以放入 Ymodem 协议解析逻辑
...
void Ymodem_RxBuffer_clean()
{
    memset(ymodem_rx_buffer, 0, YMODEM_RX_BUFFER_SIZE);
    ymodem_rx_head = 0;
    ymodem_rx_tail = 0;
    return;
}
...
uint32_t SerialKeyPressed(uint8_t *key)
{
    ...
    #else
    //printf("get: \r\n");
    if (ymodem_rx_head != ymodem_rx_tail)
    {
        *key = ymodem_rx_buffer[ymodem_rx_head++];
        printf("111 %c", *key);
        if (ymodem_rx_head >= YMODEM_RX_BUFFER_SIZE)
            ymodem_rx_head = 0;
        return 1;
    }
    //printf("\r\n");
    #endif
    // if(ymodem_rx_head/1024)
    // {
    //     printf("get\r\n");
    //     printf("%c\r\n", *key);
    //     printf("\r\n");

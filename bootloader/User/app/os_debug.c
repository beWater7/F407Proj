...
#if 0
__EXRAM uint8 __print_buf__[LINELENTH] = {0}; //保存打印内容
#else
uint8 __print_buf__[LINELENTH] = {0}; //保存打印内容
#endif
...
uint8 __printf_level__ = DLEVEL_INFO;
...
void set_print_color(int level)
{
    ...
    printf("%s", __color_output__[level]);
}
...
void os_printf_api( const char *format, ...)
{
    ...
    if(byTickFlag)
    {
        byTmpLen = sprintf((char *)__print_buf__ + byBuffLen, "[%s]", byJiffies);
        byBuffLen += byTmpLen;
    }
...
NEXT:
    ...
    {
        /* 打印内容颜色转义字符 */
        byBuffLen += sprintf((char *)__print_buf__ + byBuffLen, "%s", __color_output__[byPrintLevel]);
    }
#endif

#if DEBUGWITHPROMPT
    if(__color__)
        byBuffLen += sprintf((char *)__print_buf__ + byBuffLen, "%s", log_level_str[byPrintLevel]);
#endif
    ...
    {
        /* 写入缓存区 */
        byBuffLen += sprintf((char *)__print_buf__ + byBuffLen, "\033[0m");
    }
#endif
    ...
    {
#if DEBUGBUFFWITHCOLOR
        __print_hook(__print_buf__, byBuffLen);
#else
    ...
    if(__color__)
    {
        ...
        /* 正文 */
        printf("%s", __print_buf__);

        #if 0
        int i = 0;
        printf("--------\n");
        for(i; i < strlen(__print_buf__); i++)
        {
            printf("%c", __print_buf__[i]);
        }
        printf("--------\n");
        #endif
        /* 颜色复位 */

...
int ESP8266_WifiCredSave(void)
{
    if (setWifiStaParam(gs_esp8266_ssid, gs_esp8266_psk) != RET_OK) {
        LOGE(LOG_MOD_ESP8266, "wifi cred save to param fail\r\n");
        return -1;
    }
...
static int wifi_cred_load_legacy(void)
{
    ...
    if (crc != nv.crc) {
        LOGE(LOG_MOD_ESP8266, "wifi cred crc mismatch\r\n");
        return -1;
    }
...
int ESP8266_WifiCredLoad(void)
{
    ...
    if (getWifiStaParam(ssid, sizeof(ssid), psk, sizeof(psk)) == RET_OK && ssid[0] != '\0') {
        setEsp8266Ssid(ssid);
        setEsp8266Psk(psk);
        LOGI(LOG_MOD_ESP8266, "wifi cred loaded from param, ssid_len=%u\r\n",
             (unsigned)strlen(ssid));
        return 0;
    }
    if (wifi_cred_load_legacy() == 0) {
        (void)ESP8266_WifiCredSave();
        LOGI(LOG_MOD_ESP8266, "wifi cred migrated to param, ssid_len=%u\r\n",
...
void ESP8266_Init ( void )
{
	...
	if (ESP8266_WifiCredLoad() == 0) {
		LOGI(LOG_MOD_ESP8266, "cred ok, auto-connect after recv task starts\r\n");
	} else {
		LOGI(LOG_MOD_ESP8266, "no saved cred, use default ssid=%s\r\n",
		     getEsp8266Ssid());
	}
...
{    
	...
	/* AT 应答原样转显;归 ESP8266 模块日志,`dbg esp8266 0` 可整体静默 */
	LOGI(LOG_MOD_ESP8266, "%s", strEsp8266_Fram_Record.Data_RX_BUF);

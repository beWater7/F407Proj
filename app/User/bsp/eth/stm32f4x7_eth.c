/**
  ...
  * @version V1.1.0
  * @date    31-July-2013
  * @brief   This file is the low level driver for STM32F429xx Ethernet Controller.
...
/* FreeRTOS需要的头文件 added by ldy */
#include "FreeRTOS.h"
#include "semphr.h" 
#include "portmacro.h"


/** @addtogroup STM32F429_ETH_Driver
...
/**
  * @brief  Poll for a received frame (called from FreeRTOS task, NOT ISR).
  * @retval 1 if a complete frame is ready and ETHIN was notified, else 0.
  */
uint32_t ETH_CheckFrameReceived(void)
{
  /* last segment: complete frame available */
  if(((DMARxDescToGet->Status & ETH_DMARxDesc_OWN) == (uint32_t)RESET) &&
     ((DMARxDescToGet->Status & ETH_DMARxDesc_LS) != (uint32_t)RESET))
  {
    /* already notified, waiting for ETHIN/low_level_input to consume */
    if ((DMA_RX_FRAME_infos->Seg_Count != 0) &&
        (DMA_RX_FRAME_infos->LS_Rx_Desc == DMARxDescToGet))
    {
      return 0;
    }
    ...
    /* task context: must use xSemaphoreGive, NOT GiveFromISR */
    if (s_xSemaphore != NULL)
    {
      (void)xSemaphoreGive(s_xSemaphore);
    }
    return 1;
  }
  /* first segment */

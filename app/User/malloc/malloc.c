...
__align(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(0X6C000000 + MEM2_GLOBAL_SIZE)));					//外部SRAM内存池


//内存管理表
uint16_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(0X6C000000 + MEM2_GLOBAL_SIZE + MEM2_MAX_SIZE)));	//外部SRAM内存池MAP
...
/* 基于教程源码重新封装的代码， added by liudayi */

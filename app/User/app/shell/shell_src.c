...
static void shell_show_history(struct shell_input * shellin,int LastOrNext)
{
	...
	/* 用 ESC[2K 清行，避免单独 \\r 被串口工具拆成多行碎片提示符 */
	printk("\033[2K\r%s",shellin->sign);
...
int cmdline_param(char * str,int * argv,int maxread)
{
	...
	if (*str == '?')
		return PARAMETER_HELP;//如果命令后面的是？返回help

	for (argc = 0; *str && argc < maxread; ++argc , ++argv) { //字符不为\0的时候

...
class SerialMonitor(tk.Tk):
    ...
    def insert_ansi(self, text):
        ...
        if self.current_line_index is None:
            self.current_line_index = self.txt_output.index(tk.END)
        ...
        while pos < len(text):
            ...
            if c == '\r':
                # 覆盖当前行
                line_start = self.current_line_index
                line_end = self.txt_output.index(f"{line_start} lineend")
                logical_line = logical_line.ljust(self.prev_line_len)
                self.prev_line_len = max(self.prev_line_len, len(logical_line))
                self.txt_output.delete(line_start, line_end)
                self.txt_output.insert(line_start, logical_line, tag)
                logical_line = ''
                pos += 1
                ...
                self.txt_output.delete(line_start, line_end)
                self.txt_output.insert(line_start, logical_line, tag)
                self.txt_output.insert(f"{line_start} lineend", "\n")
                self.current_line_index = self.txt_output.index(tk.END)
                logical_line = ''
                pos += 1
